...
export default function CarPictureModal({
  ...
  const take = async () => {
    ...
    onTaken(side, pic.uri, pic.base64);
    setVisible(false);
  };
    ...
    <Modal visible={visible} animationType="fade" transparent>
      <View className="flex-1 bg-black">
        <CameraView
          ...
          mode="picture"
        />
        <View className="absolute w-full bottom-10 flex-row justify-center gap-1">
          <CustomButton
            className="bg-secondary rounded-full w-[150px] h-16 justify-center items-center"
            onPress={take}
          >
            <Ionicons name="camera" size={32} color="white" />
          </CustomButton>
          <CustomButton
            className="bg-seabed-green rounded-full w-[150px] h-16 justify-center items-center"
